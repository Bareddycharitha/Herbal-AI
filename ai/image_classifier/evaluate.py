...
def load_test_data() -> List[Tuple[Path, int]]:
    ...
    for test_dir in TEST_DIRS:
        ...
        # Special handling for Medicinal datasets
        # Both leaf and plant map to medicinal
...
def evaluate_model() -> Dict:
    ...
    try:
        ...
        for image_path, true_label in tqdm.tqdm(
            samples,
            desc="Evaluating",
        ):
        ...
        if not np.all(valid_mask):
            ...
            print(
                f"Warning: "
                f"{np.sum(~valid_mask)} images "
                f"had processing errors and "
